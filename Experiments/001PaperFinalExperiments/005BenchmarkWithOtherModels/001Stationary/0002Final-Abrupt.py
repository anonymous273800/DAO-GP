...
from Datasets.Standard.Synthetic.DS006Piecewise import DS006Piecewise
...
if __name__ == "__main__":
    ...
    X1, y1 = DS001Sin.DS001_Sinusoidal(n_samples_1, n_features, noise_level_1, lower_bound_1, upper_bound_1,
    ...
    upper_bound_2 = 8
    STRETCH_FACTOR = 1
    X2, y2 = DS007ParabolicWaveAdvanced.DS007_ParabolicWave_Simple(n_samples_2, n_features, noise_2, lower_bound_2, upper_bound_2, stretch_factor=3)
    ...
    # upper_bound_2 = 8
    #
    # X1, y1 = DS006Piecewise.DS06_Piecewise(n_samples_1, n_features, noise_1, lower_bound_1, upper_bound_1)
    # X1, y1 = shuffle(X1, y1, random_state=seed)
    # X2, y2 = DS007ParabolicWave.DS007_ParabolicWave(n_samples_2, n_features, noise_2, lower_bound_2, upper_bound_2,stretch_factor=3, weight_factor=0.3)
    # X2, y2 = shuffle(X2, y2, random_state=seed)
    #
    # DriftUtil.quantify_drift(X1, y1, X2, y2)
    #
    ...
    test_size = int(len(X2) * 0.2)
    start_test = len(X1) + len(X2) - test_size
    X_train, X_test = X[:start_test], X[start_test:]
    ...
    ################# Plotting #########################
    Plotter.plot_minibatches_results_standard_all_abrupt(KPI, x_axis_dao_gp, y_axis_dao_gp, label_dao_gp, dao_dp_final_r2,
                                                  x_axis_fitc, y_axis_fitc, label_fitc, fitc_final_r2,
                                                  x_axis_kpa, y_axis_kpa, label_kpa, kpa_r2,
                                                  X_test, y_test,
                                                  legend_loc='lower left', start_of_concept2=1000)