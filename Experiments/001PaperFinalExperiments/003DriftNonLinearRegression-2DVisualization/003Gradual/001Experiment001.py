...
def create_get_DS():
    ...
    Plotter.plot_incremental_drift((X1, y1), (X2, y2), (X3, y3), (X4, y4), (X5, y5),
                                   (X6, y6), dataset_type = 'Grad. Drift Sin')  # Works for any number of datasets!
...
if __name__ == "__main__":
    ...
    Plotter.plot_final_show_decay(
        X_base_tr, y_base_tr,
        K_inv_undecayed=K_inv,
        gamma=DECAY_GAMMA,
        dataset_type="Gradual Drift Sin",
        X_test=X_test,
        y_test=y_test,
        noise=noise,
        kernel_func=kernel_func, final_mse=final_mse, final_r2=final_r2,
        xticks_lower=-2, xticks_higher=1.75, yticks_lower=-2, yticks_hight=2.5,
        **kernel_args
    )
